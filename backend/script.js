...
const atualizarProgresso = (dispararConfete = true) => {
  ...
  Object.keys(plano).forEach((s, i) => {
    ...
    if (fill) fill.dataset.complete = (marcadosS === totalS && totalS > 0) ? 'true' : 'false';

    const texto = document.getElementById(`semana-progress-${i}`);
    if (texto) texto.textContent = `${marcadosS}/${totalS} tarefas`;

    if (fill && fill.dataset.complete === 'true' && !data.badges.includes(`Semana ${i+1} Concluída`)) {
...
const loginGoogle = async () => {
  if (loginInProgress) { showToast('Login já em andamento...'); return; }
  ...
  try {
    ...
    if (!dataLogin.url) throw new Error('Erro ao obter URL de login');
...
const logoutGoogle = () => {
  ...
  if (!token) { showToast('Nenhuma conta conectada'); return; }
...
const atualizarUsuarioLogado = async () => {
  const token = JSON.parse(localStorage.getItem('googleToken'));
  if (token) {
    dom.usuarioAvatar.src = token.photoUrl;
    dom.usuarioEmail.textContent = token.email;
    dom.usuarioAvatar.style.display = 'inline-block';
  } else {
    dom.usuarioAvatar.style.display = 'none';
    dom.usuarioEmail.textContent = '';
  }
};
...
dom.btnExportJSON.addEventListener('click', exportarJSON);
dom.btnImport.addEventListener('change', importarJSON);
